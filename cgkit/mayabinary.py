...
#        print " ", chunk
    ...
    def read(self, file):
        ...
        if len(header)!=12 or header[0:4]!="FOR4" or header[8:12]!="Maya":
            raise ValueError, 'The file "%s" is not a Maya binary file.'%self._fileName 
        ...
        while not self._abortFlag:
            ...
            if self.isGroupChunk(tag):
                ...
                if len(pendingGroups)>0 and end>pendingGroups[-1][0]:
                    raise ValueError, 'Chunk %s at position %s in file "%s" has an invalid size (%d) that goes beyond its contained group chunk.'%(tag,pos-8,os.path.basename(self._fileName),size)
    ...
    def readChunkHeader(self):
        ...
        if len(header)==0:
            return None,None
        if len(header)!=8:
            raise ValueError, 'Premature end of file "%s" (chunk tag & size expected)'%os.path.basename(self._fileName)
